...
function validateTaskQuery(request, _response, next) {
  const errors = [];
  const allowedFields = ["status", "search", "sortBy", "order", "page", "limit"];
  ...
  if (Object.hasOwn(request.query, "sortBy")) {
    if (typeof sortBy !== "string" || !allowedSortFields.includes(sortBy.trim())) {
      errors.push("sortBy query must be one of: createdAt, updatedAt, title");
    }
  }

  if (Object.hasOwn(request.query, "order")) {
    if (typeof order !== "string" || !allowedOrderValues.includes(order.trim())) {
